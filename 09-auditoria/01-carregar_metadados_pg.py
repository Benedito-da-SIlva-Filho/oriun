...
def carregar_metadados_pg(cursor, nome_tabela):
    try:
        ...
        for row in resultados:
            # Proteção contra tupla mal formatada
            if not isinstance(row, dict):
                registrar_erro(f"[METADADO] ✖ Resultado inesperado (não-DictCursor) para {nome_tabela}")
                return {"campo_chave": None, "campos": {}}
        ...
        log_execucao(f"[METADADO] 📘 Metadado carregado para {nome_tabela}: {list(metadados['campos'].keys())}")
        return metadados
